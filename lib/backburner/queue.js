...
Iterator.prototype.next = function() {
  if (this.done) {
    throw new TypeError('iterator drained');
  }

  var target                = this._queue[this._index];
  var method                = this._queue[this._index+1];
  var args                  = this._queue[this._index+2];
  var errorRecordedForStack = this._queue[this._index+3]; // Debugging assistance

  // release ?? only needed if we expect to retain the iterator and very very
  // slowly drain
  this._queue[this._index]   = null;
  this._queue[this._index+1] = null;
  this._queue[this._index+2] = null;
  this._queue[this._index+3] = null;

  this._index += 4;

  return [target, method, args, errorRecordedForStack];
};
