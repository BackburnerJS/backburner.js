...
Backburner.prototype = {
  ...
  throttle: function(target, method /* , args, wait */) {
    ...
    if (index > -1) { // throttled
      // schedule the throttler to fire again
      throttlers[index][3] = true;
      return throttlers[index];
    }

    timer = global.setTimeout(function() {
      var index = findThrottler(target, method);
      // if immediate was true then only run if it was called again
      var scheduled = throttler[3];
      if (scheduled) {
    ...
    var scheduled = immediate ? false : true;
    throttler = [target, method, timer, scheduled];
    if (immediate) {
      self.run.apply(self, args);
    }
