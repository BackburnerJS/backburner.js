...
Backburner.prototype = {
  ...
  maxTimeSlice: options && options.maxTimeSlice,

  getMaxTimeSlice: function() {
    return this.maxTimeSlice;
  },
  ...
  burnCycle: function() {
    ...
    while (!result.done && startTime > this.getTime() - this.getMaxTimeSlice()) {
      result = this.currentInstance.next();
    }

    if (!result.done){
      requestAnimationFrame(that.burnCycle);
    }
  },
  ...
  startBurnCycle: function() {
    ...
    if (this.getMaxTimeSlice()) {
      flushMethod = this.startBurnCycle;
    } else {
      flushMethod = currentInstance.flush;
    }
