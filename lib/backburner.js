...
function TagStack(name){
  this.name = name || '';
  this.opID = Debug.msTraceAsyncOperationStarting('Backburner ' + name);
  Debug.msUpdateAsyncCallbackRelation(this.opID, Debug.MS_ASYNC_CALLBACK_STATUS_JOIN);
}

TagStack.prototype.operationCompletedSuccess = function(){
  Debug.msTraceAsyncOperationCompleted(this.opID, Debug.MS_ASYNC_OP_STATUS_SUCCESS);
};

TagStack.prototype.operationCompletedError = function(){
  Debug.msTraceAsyncOperationCompleted(this.opID, Debug.MS_ASYNC_OP_STATUS_ERROR);
};

TagStack.prototype.traceCallbackStarting = function(){
  Debug.msTraceAsyncCallbackStarting(this.opID);
};

TagStack.prototype.asyncCallbackCompleted = function(){
...
Backburner.prototype = {
  ...
  run: function(target, method /*, args */) {
    ...
    if (onError) {
      try {
        var ret = method.apply(target, args);
  ...
  defer: function(queueName, target, method /* , args */) {
    ...
    var trace = new TagStack();
    var ret = this.currentInstance.schedule(queueName, target, method, args, false, stack, trace);
    trace.asyncCallbackCompleted();
    return ret;
  },
  ...
  deferOnce: function(queueName, target, method /* , args */) {
    ...
    var trace = new TagStack();
    var ret = this.currentInstance.schedule(queueName, target, method, args, true, stack, trace);
    trace.asyncCallbackCompleted();
    return ret;
  },
