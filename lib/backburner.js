...
function updateLaterTimer(self, executeAt, wait) {
  var n = now();
  if (!self._laterTimer || executeAt < self._laterTimerExpiresAt || self._laterTimerExpiresAt < n) {
    ...
    if (self._laterTimer) {
      ...
      if (self._laterTimerExpiresAt < n) { // If timer was never triggered
    ...
    self._laterTimerExpiresAt = n + wait;
  }
}
...
function executeTimers(self) {
  ...
  if (self._timers.length) {
    updateLaterTimer(self, self._timers[0], self._timers[0] - n);
  }
}
